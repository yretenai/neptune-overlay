...
inherit cmake toolchain-funcs unpacker
...
if [[ ${PV} == *9999* ]]; then
	...
	FDK_AAC_TAR="${PN}-aacdec-${FDK_AAC_COMMIT}.tar.gz"
	SRC_URI="
		...
		https://android.googlesource.com/platform/external/aac/+archive/${FDK_AAC_COMMIT}.tar.gz -> ${FDK_AAC_TAR}
	"
	SRC_A="
		${P}.tar.gz
		${PN}-ext-discord-rpc-${EXT_DISCORD_RPC_COMMIT}.tar.gz
		${PN}-ext-fmt-${EXT_FMT_COMMIT}.tar.gz
		${PN}-ext-imgui-${EXT_IMGUI_COMMIT}.tar.gz
		${PN}-ext-LibAtrac9-${EXT_LIBATRAC9_COMMIT}.tar.gz
		${PN}-ext-libusb-${EXT_LIBUSB_COMMIT}.tar.gz
		${PN}-ext-hwinfo-${EXT_HWINFO_COMMIT}.tar.gz
...
src_unpack() {
	if [[ "${PV}" == *9999* ]]; then
		...
		mv "${DISTDIR}/${FDK_AAC_TAR}" "${S}/externals/aacdec/fdk-aac" || die "Cannot move fdk-aac"
		cd "${S}/externals/aacdec/fdk-aac" && unpacker "${FDK_AAC_TAR}" || die "Cannot unpack ${FDK_AAC_TAR}"
	fi
}
